...
def unique_houses(filename):
    ...
    # first_name = file_words[0]
    # last_name =  file_words[1]
    # advisor = file_words[3]
    # cohort =file_words[4]
...
def sort_by_cohort(filename):
    ...
    for line in file_name:
        line = line.rstrip()
        file_words = line.split("|")
        name = file_words[0] + " " + file_words[1]
        cohort = file_words[4]
...
def hogwarts_by_house(filename):
    ...
    all_students.extend([gryffindor, hufflepuff, slytherin, ravenclaw,\
                        dumbledores_army, instructors, ghosts])
    file_name.close()  

    return all_students
    #we still need to alpha the students
